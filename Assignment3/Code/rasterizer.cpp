...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    // for(int i = 0; i < 3; i++){
    //     v[i] = to_vec4(view_pos[i],view_pos[i].z());
    // }
        ...
        for(float y = y_min; y < y_max + 1; y++){
            if(insideTriangle(x, y, t.v)){
                ...
                if(zp > depth_buf[get_index(x,y)]){
                    ...
                    // std::cout << interpolated_normal << std::endl << std::endl;
                    // std::cout << payload.normal.head<3>().normalized() << std::endl << std::endl;

                    // set_pixel(Vector2i(x,y),Vector3f(128,128,0));
